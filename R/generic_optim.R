#
.generic_optim<-function(opt_method, initial_pars, fcn, grad, lb, ub, params, control) {
  ...
  if (opt_method=='optim') {
    fit <- tryCatch(stats::optim(initial_pars, fn=fcn, gr=grad, method="L-BFGS-B", hessian = TRUE,
                                  lower=lb, upper=ub, params=params),
                    error=function(cond) save_fit
    )
  }
  if (opt_method=='lbfgs') {
    fit <- tryCatch(nloptr::lbfgs(x0=initial_pars, fn=fcn, gr=grad,
                                  lower=lb, upper=ub, params=params, control=nlopt_control),
                    error=function(cond) save_fit
    )
  }
  if (opt_method=='slsqp') {
    fit <- tryCatch(nloptr::slsqp(x0=initial_pars, fn=fcn, gr=grad, lower=lb, upper=ub,
  ...
  if (opt_method=='BBoptim') {
    fit <- tryCatch(BB::BBoptim(par=initial_pars, fn=fcn, gr=grad, lower=lb, upper=ub,
                                control=control, params=params, quiet=TRUE),
                    error=function(cond) save_fit
    )
  }
  if (opt_method=='psoptim') {
    ps_control<-list(maxit=control$maxit,trace=control$trace,hybrid=TRUE)
    fit <- tryCatch(pso::psoptim(par=initial_pars, fn=fcn, gr=grad, lower=lb, upper=ub, params=params, control=ps_control),
                    error=function(cond) save_fit
    )
  }
...
.best_optim <- function(initial_pars, fcn, grad, lb, ub, params, control) {
  best_val    <- Inf
  best_method <- 'none'
  best_fit    <- list()
  for (opt_method in .supported_optim()) {
    fit <- .generic_optim(opt_method=opt_method, initial_pars=initial_pars, fcn=fcn, grad=grad,
...
.generic_optim_list <- function(opt_method_list, initial_pars, fcn, grad,
